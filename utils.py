...
conversion_prompt = """
    ...
    6. Replace `{{USERID}}` and `{{PVDATE}}` with `{{user_id}}` and `{{pv_date}}`. Ennsure That either of them are enclosed in double brackets
    7. Comparisson with pv_date should always be col_name </>/= epoch_ms({{pvdate}})
    Example: 
    Input MySQL Query:
...
def convert_query(query):
    ...
    query_output = query_output.replace("{user_id}", "{{user_id}}").replace("{pv_date}", "{{pv_date}}")
